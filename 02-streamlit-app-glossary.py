...
# Load Sign Names CSV
sign_names_df = pd.read_csv('data/Sign Names.csv')

# Function to create glossary
def create_glossary():
    st.subheader("Traffic Sign Glossary")
    cols = st.columns(4)
    for i, row in sign_names_df.iterrows():
        with cols[i % 4]:
            img_path = f"data/Meta/{row['ClassId']}.png"
            img = Image.open(img_path)
            st.image(img, width=50, caption=f"{row['ClassId']}: {row['SignName']}")
...
5. Check the glossary below for a quick reference of all traffic signs.
